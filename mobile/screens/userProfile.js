...
const UserProfile = ({ navigation, route }) => {
    ...
    const fetchUser = async () => {
        setLoading(true);  // وقتی شروع به درخواست می‌کنی loading فعال می‌شه
        try {
            ...
            const response = await axios.get(api.openUserProfileGet, {
                params: { email },
                headers: { "auth-token": `Bearer ${token}` }
            });
            if (!response || response.status !== 200 || !response.data.data) {
                console.log('Invalid response or no data');
                // اینجا می‌تونی پیام خطا نمایش بدی یا حالت مناسب دیگه
                return;
            }
            ...
            setLoading(false);  // در نهایت loading خاموش می‌شه
        }
    };
        ...
        <SafeAreaView style={styles.SafeArea} >
            ...
            <View style={styles.container}>
                ...
                <View style={styles.header}>
                    ...
                    <Text style={styles.headerTitle}>{user.name || "Unkown"}</Text>
