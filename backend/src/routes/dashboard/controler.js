...
class Dashboard {
    infoSelfProfile = async (req, res) => {
        try {
            ...
            res.status(200).json({message : "successfuly to get your data " , data : {name : user.name , email : user.email , posts : user.posts , followers : user.followers , following : user.following , followersList : user.followerUser , followingList : user.followingUser}});
        }
        catch (e) {
